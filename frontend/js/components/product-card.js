var ProductCard = {
  render: function(p) {
    ...
    var imgEmoji = p.image && p.image.length <= 6 ? p.image : (emojiMap[p.category] || "📦");
    var imageValue = p.image || imgEmoji;
    var imageHtml = p.image && p.image.length > 6
    ...
    return '<div class="tb-product-card" onclick="showToast(' + "'" + p.name + ' - ¥' + p.price + "'" + ',info)">' +
      ...
      '<button class="btn btn-sm" style="width:100%;margin-top:6px;justify-content:center;" onclick="event.stopPropagation();App.addToCart({id:' + "'" + (p.id || "") + "'" + ',name:' + "'" + (p.name || "") + "'" + ',price:' + (p.price || 0) + ',image:' + "'" + imageValue + "'" + ',shop_name:' + "'" + (p.shop_name || "") + "'" + '})"><i class="fas fa-cart-plus"></i> 加入购物车</button></div></div>';
  }
};
