var VueProductList = {
  ...
  template: '<div>' +
    ...
    '<div class="prod-img"><img v-if="p.image&&p.image.length>6" :src="p.image" style="width:100%;height:100%;object-fit:cover;" :alt="p.name" /><span v-else>{{ getEmoji(p) }}</span><span v-if="p.certification" class="prod-tag">{{ p.certification }}</span></div>' +
