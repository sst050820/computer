var VueEmergency = {
  ...
  template: '<div>' +
    ...
    '<button @click="doDecrypt" :disabled="loading||!productId" style="display:inline-flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:14px;border:none;border-radius:var(--rd-md);font-size:1rem;font-weight:700;cursor:pointer;background:linear-gradient(135deg,#C53030,#D14343);color:#fff;box-shadow:0 4px 16px rgba(197,48,48,0.35);transition:all 0.2s;" onmouseover="this.style.transform=\'translateY(-1px)\';this.style.boxShadow=\'0 6px 22px rgba(197,48,48,0.45)\'" onmouseout="this.style.transform=\'none\';this.style.boxShadow=\'0 4px 16px rgba(197,48,48,0.35)\'"><i v-if="loading" class="fas fa-spinner fa-spin"></i><i v-else class="fas fa-unlock"></i> {{ loading?"解密中...":"确认紧急解密（操作将记录并上报）" }}</button>' +
